...
const Page = () => {
    ...
    const apiUrl = `/api/history/quiz-generator?limit=${PAGE_SIZE}&offset=${(queryPage - 1) * PAGE_SIZE}&search=${encodeURIComponent(debouncedSearch)}`;

    // Fetch data using SWR
    const { data, error, isLoading } = useSWR<QuizGeneratorResponse>(apiUrl, fetcher, {
                                    ...
                                    <TableRow key={index}>
                                        ...
                                        <TableCell>
                                            <ResponseModal
                                                ...
                                                content={
                                                    record.question_types && record.question_types.length > 0
                                                        ? record.question_types.join(", ")
                                                        : "None"
                                                }
                                            />
                                        </TableCell>
                                        ...
                                        <TableCell>
                                            <ResponseModal
                                                title="Quiz Data"
                                                content={JSON.stringify(record.quiz_data, null, 2)}
                                            />
                                        </TableCell>
