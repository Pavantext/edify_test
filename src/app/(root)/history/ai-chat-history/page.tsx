...
const Page = () => {
    ...
    const apiUrl = `/api/history/chat-sessions?limit=${PAGE_SIZE}&offset=${(queryPage - 1) * PAGE_SIZE}&search=${encodeURIComponent(debouncedSearch)}`;

    // Fetch data using SWR
    const { data, error, isLoading } = useSWR<ChatSessionResponse>(apiUrl, fetcher, {
                                ...
                                history.map((record, index) => (
                                    <TableRow key={index}>
                                        ...
                                        <TableCell>
                                            <ResponseModal
                                                title="Messages"
                                                content={JSON.stringify(record.messages, null, 2)}
                                            />
                                        </TableCell>
                                        <TableCell>
