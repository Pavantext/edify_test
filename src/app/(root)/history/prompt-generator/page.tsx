...
const Page = () => {
    ...
    const apiUrl = `/api/history/prompt-generator?limit=${PAGE_SIZE}&offset=${(queryPage - 1) * PAGE_SIZE}&search=${encodeURIComponent(debouncedSearch)}`;

    // Fetch data using SWR
    const { data, error, isLoading } = useSWR<PromptGeneratorResponse>(apiUrl, fetcher, {
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow>
                                ...
                                <TableHead>Generation Model</TableHead>
                                <TableHead>Processing Time (ms)</TableHead>
                                <TableHead>Input Original Prompt</TableHead>
                                <TableHead>AI Refined Prompts</TableHead>
                            ...
                            {history.length > 0 ? (
                                history.map((record, index) => (
                                    <TableRow key={index}>
                                        ...
                                        <TableCell>{record.generation_model}</TableCell>
                                        <TableCell>{record.processing_time_ms}</TableCell>
                                        <TableCell>
                                            <ResponseModal
                                                title="Input Original Prompt"
                                                content={record.input_original_prompt}
                                            />
                                        </TableCell>
                                        <TableCell>
                                            <ResponseModal
                                                title="AI Refined Prompts"
                                                content={JSON.stringify(record.ai_refined_prompts, null, 2)}
                                            />
                                        </TableCell>
