...
export async function GET(req: Request): Promise<NextResponse> {
    try {
        ...
        // Fetch the rubrics generator records.
        ...
        if (resultsError) {
            console.error("Error fetching rubrics generator results:", resultsError);
            return NextResponse.json(
                { error: "Failed to fetch rubrics generator results" },
                { status: 500 }
            );
        }

        // Retrieve usernames from the users table.
        const uniqueUserIds = Array.from(new Set(results.map((record: any) => record.user_id)));
        ...
        // Format the data; if assignment_type is missing, fallback to custom_assignment_type.
        const formattedData = results.map((record: any) => {
            let assignment = record.assignment_type;
            if (!assignment || assignment.trim() === "") {
                assignment = record.custom_assignment_type;
            }
            return {
                username: userMap.get(record.user_id) || "Unknown",
                assignment,
                key_stage: record.key_stage,
                year_group: record.year_group,
                assessment_type: record.assessment_type,
                criteria: record.criteria,
                additional_instructions: record.additional_instructions,
